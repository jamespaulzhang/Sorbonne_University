            ...
            try {
                ...
                if (await users.get(uid) != null) {
                    if(req.session.uid !== uid) {
                        ...
                        res.status(200).json({
                            status: 200,
                            message: "Succès : L'utilisateur est supprimé."
                        });
                    }
                    else {
                        res.status(403).json({
                            status: 403,
                            message: "Erreur : La session est en cours d'utilisation."
                        });
                    }
                }
                else {
    ...
    router.route('/reply')
        ...
        .put(async (req, res) => {
            try {
                ...
                let { content, mid, uid } = req.body;

                if (content != null && mid != null && uid != null) {
                    if (await messages.get(mid) != null) {
                        if (await users.get(uid) != null) {
                            let rid = await replies.create(content, mid, uid);
                            res.status(201).json({
                                status: 201,
