...
def gale_shapley_etudiants(CE, CP, capacites):
    ...
    # 1. Trouver un étudiant libre : deque permet un accès rapide en O(1) pour chaque opération
    etudiants_libres = deque(range(n))  # Initialisation en O(n)

    # 2. Prochain parcours à proposer : tableau pour accès direct en O(1)
    prochain_parcours = [0] * n  # Initialisation en O(n)

    # 3. Classement des étudiants pour chaque parcours : dictionnaire pour accès rapide en O(1)
    classement_parcours = [{etudiant: rang for rang, etudiant in enumerate(CP[j])} for j in range(m)]  # Initialisation en O(n*m)

    # 4. Affectations des parcours : heapq pour trouver l'étudiant le moins préféré en O(log k) par insertion/suppression
    affectations = {j: [] for j in range(m)}

    # 5. Capacités restantes pour chaque parcours : tableau pour accès direct en O(1)
    capacite_restante = capacites[:]  # Initialisation en O(m)

    # Boucle principale : tant qu'il y a des étudiants libres
    while etudiants_libres:
        i = etudiants_libres.popleft()  # O(1) Sélectionner un étudiant libre
        
        # Boucle pour chaque étudiant : proposer à tous les parcours dans l'ordre de préférence
        while prochain_parcours[i] < m:
            j = CE[i][prochain_parcours[i]]  # O(1) Parcours préféré actuel de l'étudiant i
            prochain_parcours[i] += 1 # Passer au parcours suivant pour cet étudiant

            # Si le parcours j a de la capacité restante, ajouter l'étudiant à la file de priorité du parcours
            if capacite_restante[j] > 0:
                ...
                # Si le parcours est plein, vérifier si l'étudiant est mieux classé que l'étudiant le moins préféré
                moins_prefere_rang, moins_prefere = affectations[j][0]  # O(1)
                
                # Si l'étudiant i est mieux classé, remplacer l'étudiant le moins préféré
                if classement_parcours[j][i] < -moins_prefere_rang:
                    heapq.heappop(affectations[j])  # O(log k)
                    etudiants_libres.append(moins_prefere)  # O(1)
                    heapq.heappush(affectations[j], (-classement_parcours[j][i], i))  # O(log k)
                    break

    # Construire le résultat final des affectations
    return {j: [etudiant for _, etudiant in sorted(affectations[j])] for j in range(m)}
...
def gale_shapley_parcours(CE, CP, capacites):
    ...
    # Utilisation d'un min-heap pour gérer les parcours libres efficacement
    parcours_libres = list(range(m))  # O(m) : Conversion en liste pour heapq
    heapq.heapify(parcours_libres)  # O(m)

    # Initialisation des structures
    capacite_restante = capacites[:]  # O(m)
    etudiant_affecte = [-1] * n  # O(n)

    # Optimisation : dictionnaire pour accès rapide aux préférences des étudiants
    classement_etudiants = [{parcours: rang for rang, parcours in enumerate(CE[i])} for i in range(n)]  # O(n*m)

    # Optimisation : dictionnaire avec `deque` pour éviter `pop(0)` coûteux
    CP = {i: deque(CP[i]) for i in range(m)}  # O(m)

    # Optimisation : utilisation de `set` pour des suppressions rapides
    affectations = {j: set() for j in range(m)}  # O(m)

    while parcours_libres:
        parcours = heapq.heappop(parcours_libres)  # O(log m)

        while capacite_restante[parcours] > 0 and CP[parcours]:
            etudiant = CP[parcours].popleft()  # O(1)

            if etudiant_affecte[etudiant] == -1:
                # Affecter l'étudiant au parcours
                etudiant_affecte[etudiant] = parcours
                affectations[parcours].add(etudiant)  # O(1)
                ...
                if classement_etudiants[etudiant][parcours] < classement_etudiants[etudiant][ancien_parcours]:
                    # L'étudiant préfère ce nouveau parcours
                    affectations[ancien_parcours].remove(etudiant)  # O(1) avec `set`
                    capacite_restante[ancien_parcours] += 1
                    affectations[parcours].add(etudiant)  # O(1)
                    capacite_restante[parcours] -= 1
                    etudiant_affecte[etudiant] = parcours
                    heapq.heappush(parcours_libres, ancien_parcours)  # O(log m)

        if capacite_restante[parcours] > 0:
            heapq.heappush(parcours_libres, parcours)  # O(log m)

    # Convertir affectations en liste pour garder le format d'origine
    return {k: list(v) for k, v in affectations.items()}
...
def maximize_utility_and_fairness(n, m, preferences_students, preferences_parcours, capacities, k):
    ...
    if pulp.LpStatus[problem.status] == "Optimal":
        ...
        average_utility = total_utility_value / (n+m)
...
def main():
    while True:
        ...
        print("7. Verifier la stabilité des affectations")
        print("8. Quitter")
