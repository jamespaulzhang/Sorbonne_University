...
def main(nb_parties, nb_jours):
    iterations = 60  # nb de pas max par épisode
    if len(sys.argv) == 2:
    ...
    strategy_names = []
    choix_initiaux = {}
    distance_vision = float('inf')  # Laisser l'utilisateur la définir
    ...
    path = [[] for _ in range(nb_players)]
    ...
    for _ in range(nb_parties):
        ...
        for day in range(nb_jours):
            ...
            # Initialisation des choix de restaurant
            choix_resto = [None] * nb_players
            for p, strategy in enumerate(strategies):
                if strategy_names[p] == "Séquence Fixe":
                    choix_resto[p] = strategy(day_param=day)
                elif strategy_names[p] != "Humaine":  # Ne pas initialiser pour "Humaine"
                    choix_resto[p] = strategy()
            ...
            # Calcul des chemins initiaux pour les joueurs ayant déjà une destination
            for p in range(nb_players):
                if choix_resto[p]:  # Pour les joueurs non-humains
                    prob = ProblemeGrid2D(players[p].get_rowcol(), choix_resto[p], g, 'manhattan')
                    path.append(probleme.astar(prob, verbose=False))
                else:
                    path.append([])  # Chemin vide initial pour le joueur humain
            ...
            for i in range(iterations):
                ...
                # Moment pour que le joueur humain choisisse son restaurant
                if i == iterations // 2:
                    for p in range(nb_players):
                        if strategy_names[p] == "Humaine":
                            current_pos = players[p].get_rowcol()
                            choix_resto[p] = human_behavior(pos_restaurants, nb_players_in_resto, current_pos, distance_vision, temps_restant[p])
                            print(f"Joueur {p+1} (humain) choisit sa destination: {choix_resto[p]}")

                            # Calcul du nouveau chemin
                            prob = ProblemeGrid2D(current_pos, choix_resto[p], g, 'manhattan')
                            path[p] = probleme.astar(prob, verbose=False)

                            # Réinitialisation du temps restant
                            temps_restant[p] = iterations - i

                # Déplacement des joueurs
                for p in range(nb_players):
                    if choix_resto[p]:  # Si le joueur a une destination
                        # Gestion du temps écoulé en premier
                        temps_restant[p] -= 1

                        # Vérifier si le temps est écoulé avant le déplacement
                        if temps_restant[p] <= 0:
                            print(f"ALERTE: Le temps du joueur {p+1} est écoulé !")

                            # Gestion des stratégies spécifiques
                            if strategy_names[p] == "Greedy":
                                new_choice = strategie_greedy(
                                    pos_restaurants,
                                    nb_players_in_resto,
                                    seuils[p],
                                    players[p].get_rowcol(),
                                ...
                                if new_choice != choix_resto[p]:
                                    print(f"Joueur {p+1} change de restaurant: {choix_resto[p]} ➝ {new_choice}")
                                    choix_resto[p] = new_choice
                                    current_pos = players[p].get_rowcol()
                                    prob = ProblemeGrid2D(current_pos, choix_resto[p], g, 'manhattan')
                                    path[p] = probleme.astar(prob, verbose=False)
                        ...
                        # Pour le joueur humain seulement
                        if strategy_names[p] == "Humaine":
                            if i < iterations // 2:  # Avant le choix
                                continue  # Ne pas bouger
                            else:  # Après le choix
                                path_index = i - (iterations // 2)  # Index relatif
                        else:  # Pour tous les autres joueurs
                            path_index = i  # Index absolu

                        # Déplacement normal
                        if 0 <= path_index < len(path[p]):
                            (row, col) = path[p][path_index]
                            players[p].set_rowcol(row, col)
                            print(f"Joueur {p+1} | Position: ({row}, {col}) | Destination: {choix_resto[p]}")

                        # Gestion des coupe-files
                        for cf in coupe_files:
            ...
            for p in range(nb_players):
                if choix_resto[p] is not None and choix_resto[p] in pos_restaurants:
